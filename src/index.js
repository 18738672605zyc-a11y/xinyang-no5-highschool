/**
 * 信阳市第五高级中学官网 - Cloudflare Workers 后端
 */
...
async function handleRequest(request, env, ctx) {
  ...
  try {
    ...
    if (path === '/announcements' && method === 'GET') {
      return handleGetAnnouncements(request, db, url);
    }
    ...
    if (err.message === 'Forbidden') return json({ success: false, message: '无权限操作' }, 403);
...
async function handleLogin(request, db) {
  ...
  return json({ success: true, token, user: { id: user.id, username: user.username } });
}

// ---- Handler: 获取公告列表 ----
async function handleGetAnnouncements(request, db, url) {
